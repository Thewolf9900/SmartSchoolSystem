...
function ChatModal({ show, onHide }) {
    ...
    return (
        <Modal show={show} onHide={onHide} size="lg" centered backdrop="static" className="chat-modal-modern">
            <Modal.Header closeButton style={{
                ...
                <Modal.Title className="d-flex align-items-center" style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>
                    <i className="fas fa-robot me-2 text-white"></i> المساعد الذكي
                </Modal.Title>
                <div className="ms-auto d-flex align-items-center gap-2">
                    <Button variant="link" size="sm" onClick={handleClearConversation} title="مسح المحادثة الحالية" className="text-white p-1 hover-scale">
                        <i className="fas fa-eraser"></i>
                    </Button>
                    <OverlayTrigger trigger="click" placement="bottom-end" overlay={settingsPopover} rootClose>
                        <Button variant="link" size="sm" title="تخصيص المظهر" className="text-white p-1 hover-scale">
            ...
            <Modal.Body style={{ display: 'flex', flexDirection: 'column', height: '65vh', padding: '0' }}>
                {isLoadingConvs ? (
                    <div className="d-flex justify-content-center align-items-center h-100 flex-column" style={{ minHeight: '300px' }}>
                        <Spinner animation="grow" variant="primary" />
                        <span className="mt-3 text-muted font-weight-bold">جاري تحميل المحادثات...</span>
                    </div>
                ) : (
                    <div className="bg-light px-2 pt-2 border-bottom">
                        <Tabs activeKey={activeConvId} onSelect={handleTabSelect} className="chat-tabs border-0" variant="tabs">
                            {conversations.map((conv) => (
                                <Tab key={conv.id} eventKey={conv.id} title={
                                    <div className="d-flex align-items-center">
                                        <span style={{ fontSize: '0.9rem' }}>{conv.name || `محادثة ${conv.id}`}</span>
                                        <Button variant="link" size="sm" className="ms-2 p-0 text-muted close-tab-btn hover-danger" onClick={(e) => handleCloseConversation(e, conv.id)} style={{ lineHeight: 1, textDecoration: 'none' }}>&times;</Button>
                                    </div>
                                } />
                            ))}
                            <Tab eventKey="new-tab" title={<span className="font-weight-bold" style={{ fontSize: '1.2rem', lineHeight: '1' }}>+</span>} disabled={conversations.length >= 3} />
                        </Tabs>
                    </div>
                )}
            ...
            <Modal.Footer style={{ ...dynamicStyles.footer, borderTop: '1px solid #dee2e6', padding: '15px' }}>
                <Form className="w-100 d-flex gap-2 align-items-center" onSubmit={handleSendMessage}>
                    <Form.Control
                        ref={messageInputRef}
                        type="text"
